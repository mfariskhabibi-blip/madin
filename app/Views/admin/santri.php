...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .u-avatar { width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: .8rem; font-weight: 700; color: #fff; flex-shrink: 0; background: var(--purple); }
        .u-info .u-name { font-size: .875rem; font-weight: 600; color: var(--dark); }
        .u-info .u-nis { font-size: .75rem; color: var(--gray); }
        ...
        .badge-status-lulus { background: rgba(14,165,233,.1); color: var(--info); }
        .badge-status-lulus .dot { background: var(--info); }
        .badge-status-dropped { background: rgba(229,62,62,.1); color: var(--danger); }
        .badge-status-dropped .dot { background: var(--danger); }
        ...
        .modal-card { background: #fff; width: 100%; max-width: 550px; max-height: 90vh; overflow-y:auto; border-radius: var(--radius); box-shadow: 0 10px 30px rgba(0,0,0,.2); animation: slideUp .3s ease; }
...
<body>
    ...
    <div class="dashboard-content" id="mainContent">
        <h1 class="page-title"><i class="fas fa-user-graduate"></i> Data Santri</h1>
        ...
        <div class="section-card">
            <div class="card-header">
                <div class="card-title">
                    <div style="width:36px;height:36px;border-radius:8px;background:rgba(139,92,246,.1);color:var(--purple);display:flex;align-items:center;justify-content:center;">
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                                ...
                                <tr>
                                    <td data-label="Profil Santri">
                                        <div class="user-cell">
                                            <div class="u-avatar">
                                                <?php
                                                    $i = ''; $p = explode(' ', $s['nama_santri'] ?? 'S');
                                                    foreach($p as $w) { $i .= strtoupper(substr($w,0,1)); if(strlen($i)>=2) break; }
                                                    echo $i ?: 'S';
                                                ?>
                                            </div>
                                            <div class="u-info">
                                                <div class="u-name"><?= htmlspecialchars($s['nama_santri'] ?? '-') ?></div>
                                    ...
                                    <td data-label="L/P">
                                        <span class="badge badge-gender">
                                            <i class="fas <?= $s['jenis_kelamin'] == 'L' ? 'fa-mars' : 'fa-venus' ?>"></i>
                                            <?= $s['jenis_kelamin'] == 'L' ? 'Pria' : 'Wanita' ?>
                                        </span>
                                    </td>
                                    <td data-label="Nama Wali (Ortu)">
                                        <span style="font-weight:600;font-size:.85rem;"><?= htmlspecialchars($s['nama_ortu'] ?? 'Belum Ditautkan') ?></span>
                                    </td>
                                    <td data-label="Status">
                                        <?php 
                                            $bClass = 'badge-status-aktif';
                                            if($s['status'] == 'lulus') $bClass = 'badge-status-lulus';
                                            if($s['status'] == 'drop out') $bClass = 'badge-status-dropped';
                                        ?>
                                        <span class="badge <?= $bClass ?>">
                                            <span class="dot"></span><?= ucfirst($s['status']) ?>
                                        </span>
                                    </td>
                                    <td data-label="Aksi">
                                        <div class="action-cell">
                                            ...
                                            <a href="<?= base_url('admin/santri/delete/' . $s['id']) ?>" class="act-btn act-delete" onclick="return confirm('Hapus data santri ini beserta nilainya?')" title="Hapus">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="6" style="text-align:center;padding:30px;color:var(--gray);">Belum ada data santri yang didaftarkan.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
    ...
    <div class="modal-overlay" id="addModal">
        <form action="<?= base_url('admin/santri/store') ?>" method="post" class="modal-card">
            <?= csrf_field() ?>
            ...
            <div class="modal-body">
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="nis" class="form-control" required placeholder="Contoh: 10293847">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Nama Lengkap Santri *</label>
                        <input type="text" name="nama_santri" class="form-control" required placeholder="Masukkan nama lengkap">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea name="alamat" class="form-control" rows="2" placeholder="Alamat rumah santri..."></textarea>
                </div>
            </div>
        ...
        <form id="editForm" method="post" class="modal-card">
            ...
            <div class="modal-body">
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label class="form-label">Nama Lengkap Santri *</label>
                        <input type="text" name="nama_santri" id="edit_nama" class="form-control" required>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
...
<script>
    ...
    function openEditModal(santri) {
        ...
        document.getElementById('edit_nama').value = santri.nama_santri || '';
