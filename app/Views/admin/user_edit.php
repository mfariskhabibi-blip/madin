...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .header-meta { display: flex; gap: 20px; margin-top: 8px; flex-wrap: wrap; }
        .header-meta-item { display: flex; align-items: center; gap: 6px; font-size: 0.85rem; opacity: 0.8; }
        .header-meta-item i { font-size: 0.75rem; }
        ...
        /* STATUS BADGE */
        .status-badge { display: inline-flex; align-items: center; gap: 6px; padding: 4px 14px; border-radius: 20px; font-size: 0.75rem; font-weight: 600; }
        .status-aktif { background: rgba(38,162,105,.12); color: #1e8555; }
        .status-nonaktif { background: rgba(229,62,62,.12); color: #c53030; }
        ...
        .santri-entry { position: relative; background: #fafbfc; border: 2px solid var(--light-gray); border-radius: 12px; padding: 24px; margin-bottom: 24px; transition: var(--transition); }
        ...
        /* INFO CARD */
        .info-card { background: white; border-radius: 12px; box-shadow: var(--shadow); overflow: hidden; margin-top: 30px; }
        .info-card-header {
        ...
        .info-card-body li { font-size: 0.85rem; color: var(--gray); margin-bottom: 8px; display: flex; align-items: flex-start; gap: 8px; }
        .info-card-body li i { color: var(--info); margin-top: 3px; font-size: 0.75rem; }
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="dashboard-content" id="mainContent">
            ...
            <?php
                $userInitials = '';
                $uWords = explode(' ', $user['nama_lengkap']);
                foreach($uWords as $w) { $userInitials .= strtoupper(substr($w, 0, 1)); if(strlen($userInitials) >= 2) break; }
            ?>
            <div class="page-header-card">
                <div class="header-icon" style="font-size: 1.6rem; font-weight: 700;">
                    <?= $userInitials ?: 'U' ?>
                </div>
                <div class="header-text">
                    ...
                    <div class="header-meta">
                        ...
                        <div class="header-meta-item">
                            <i class="fas fa-circle" style="font-size:0.5rem; color: <?= $user['status'] == 'aktif' ? 'var(--success)' : 'var(--danger)' ?>;"></i>
                            <?= ucfirst($user['status']) ?>
                        </div>
                        <?php if(!empty($user['created_at'])): ?>
                            <div class="header-meta-item"><i class="fas fa-calendar"></i> Dibuat: <?= date('d M Y', strtotime($user['created_at'])) ?></div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>

            <form action="<?= base_url('admin/users/update/' . $user['id']) ?>" method="POST" id="editUserForm">
                <?= csrf_field() ?>
                    ...
                    <div class="form-card-body">
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="nama_lengkap" placeholder="Masukkan nama lengkap pengguna" value="<?= old('nama_lengkap') ?? $user['nama_lengkap'] ?>" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label">
                                    <i class="fas fa-at"></i> Username <span class="required">*</span>
                                </label>
                                <input type="text" class="form-control" name="username" placeholder="Masukkan username untuk login" value="<?= old('username') ?? $user['username'] ?>" required>
                            ...
                            <div class="form-group">
                                ...
                                <input type="email" class="form-control" name="email" placeholder="contoh@email.com (opsional)" value="<?= old('email') ?? $user['email'] ?>">
                            </div>
                            <div class="form-group">
                                <label class="form-label">
                                ...
                                <div class="password-wrapper">
                                    <input type="password" class="form-control" name="password" id="passwordField" placeholder="Kosongkan jika tidak diubah">
                                    <button type="button" class="password-toggle" onclick="togglePassword()">
                                        <i class="fas fa-eye" id="passwordIcon"></i>
                                    </button>
                                </div>
                                <div class="form-hint"><i class="fas fa-shield-alt"></i> Isi hanya jika ingin mengubah password. Minimal 5 karakter.</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-card">
                    ...
                    <div class="form-card-body">
                        ...
                        <div class="role-cards">
                            <label class="role-card admin-card <?= $currentRole === 'admin' ? 'selected' : '' ?>">
                                <input type="radio" name="role" value="admin" <?= $currentRole === 'admin' ? 'checked' : '' ?> required>
                            ...
                            <label class="role-card ustadz-card <?= $currentRole === 'ustadz' ? 'selected' : '' ?>">
                                <input type="radio" name="role" value="ustadz" <?= $currentRole === 'ustadz' ? 'checked' : '' ?>>
                                <div class="check-mark"><i class="fas fa-check"></i></div>
                            ...
                            <label class="role-card ortu-card <?= $currentRole === 'ortu' ? 'selected' : '' ?>">
                                <input type="radio" name="role" value="ortu" <?= $currentRole === 'ortu' ? 'checked' : '' ?>>
                                <div class="check-mark"><i class="fas fa-check"></i></div>
                ...
                <!-- CONDITIONAL: ORTU SECTIONS -->
                <div class="ortu-sections <?= $user['role'] === 'ortu' ? 'active' : '' ?>" id="ortuSections">
                        ...
                        <div class="form-card-body">
                            <div class="form-grid">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="nama_ayah" placeholder="Masukkan nama ayah" value="<?= old('nama_ayah', $parent['nama_ayah'] ?? '') ?>">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">
                                        <i class="fas fa-female"></i> Nama Ibu
                                    </label>
                                    <input type="text" class="form-control" name="nama_ibu" placeholder="Masukkan nama ibu" value="<?= old('nama_ibu', $parent['nama_ibu'] ?? '') ?>">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">
                                        <i class="fas fa-phone-alt"></i> No. HP Ayah
                                    </label>
                                    <input type="text" class="form-control" name="no_telepon_ayah" placeholder="Contoh: 08123456xxxx" value="<?= old('no_telepon_ayah', $parent['no_telepon_ayah'] ?? '') ?>">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">
                                        <i class="fas fa-phone-alt"></i> No. HP Ibu
                                    </label>
                                    <input type="text" class="form-control" name="no_telepon_ibu" placeholder="Contoh: 08123456xxxx" value="<?= old('no_telepon_ibu', $parent['no_telepon_ibu'] ?? '') ?>">
                                </div>
                                <div class="form-group full-width">
                                    <label class="form-label">
                                        <i class="fas fa-map-marker-alt"></i> Alamat Orang Tua
                                    </label>
                                    <input type="text" class="form-control" name="alamat_ortu" placeholder="Alamat lengkap orang tua" value="<?= old('alamat_ortu', $parent['alamat'] ?? '') ?>">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-card">
                        ...
                        <div class="form-card-body">
                            <div id="santriContainer">
                                <?php 
                                $santriData = old('santri') ?? $santri ?? [];
                                if (empty($santriData)) {
                                    $santriData = [[]]; // Default minimal 1 entry if none exist
                                }
                                ?>
                                <?php foreach ($santriData as $idx => $s): ?>
                                    <div class="santri-entry" data-index="<?= $idx ?>">
                                        ...
                                        <div class="form-grid">
                                            <div class="form-group full-width">
                                                ...
                                                <input type="text" class="form-control" name="santri[<?= $idx ?>][nama_santri]" placeholder="Nama lengkap anak" value="<?= $s['nama_santri'] ?? '' ?>">
                                            </div>
                                            <div class="form-group">
                                                <label class="form-label">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" name="santri[<?= $idx ?>][tempat_lahir]" placeholder="Kota/Kabupaten" value="<?= $s['tempat_lahir'] ?? '' ?>">
                                            </div>
                                            <div class="form-group">
                                                <label class="form-label">
                                                    <i class="fas fa-calendar-alt"></i> Tanggal Lahir <span class="required">*</span>
                                                </label>
                                                <input type="date" class="form-control" name="santri[<?= $idx ?>][tanggal_lahir]" value="<?= $s['tanggal_lahir'] ?? '' ?>">
                                            </div>
                                            <div class="form-group">
                                                <label class="form-label">
                                                    <i class="fas fa-home"></i> Alamat Lengkap
                                                </label>
                                                <input type="text" class="form-control" name="santri[<?= $idx ?>][alamat]" placeholder="Alamat lengkap santri" value="<?= $s['alamat'] ?? '' ?>">
                                            </div>
                                            <div class="form-group">
                                                <label class="form-label">
                                                    <i class="fas fa-graduation-cap"></i> Tahun Angkatan
                                                </label>
                                                <input type="number" class="form-control" name="santri[<?= $idx ?>][tahun_angkatan]" placeholder="<?= date('Y') ?>" value="<?= $s['tahun_angkatan'] ?? date('Y') ?>" min="2000" max="2099">
                                            </div>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
            ...
            <div class="info-card">
                <div class="info-card-header">
                    ...
                    <h4>Panduan Edit Akun</h4>
                </div>
                <div class="info-card-body">
                    <ul>
                        <li><i class="fas fa-circle"></i> Password hanya diisi jika ingin melakukan perubahan (minimal 5 karakter).</li>
                        <li><i class="fas fa-circle"></i> Jika peran diubah menjadi <strong>Orang Tua</strong>, sistem akan meminta data profiling tambahan.</li>
                        <li><i class="fas fa-circle"></i> NIS santri baru akan dihasilkan secara otomatis oleh sistem.</li>
                        <li><i class="fas fa-circle"></i> Pastikan semua field bertanda <span class="required">*</span> terisi dengan benar.</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        // Dynamic Santri management
        let santriIndex = <?= max(0, count($santriData) - 1) ?>;
            ...
            entry.innerHTML = `
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" name="santri[${idx}][tahun_angkatan]" value="<?= date('Y') ?>" min="2000" max="2099">
                    </div>
                </div>
            `;
            santriContainer.appendChild(entry);
            // Smooth scroll to new entry
            entry.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }

        function removeSantriEntry(btn) {
            const entry = btn.closest('.santri-entry');
            if (confirm('Apakah Anda yakin ingin menghapus data santri ini dari form?')) {
        ...
        function reindexSantri() {
            const entries = santriContainer.querySelectorAll('.santri-entry');
            entries.forEach((e, i) => {
                e.dataset.index = i;
                e.querySelector('.entry-num').textContent = i + 1;
                e.querySelector('h4').lastChild.textContent = ` Santri ke-${i + 1}`;
                
                // Update names of all inputs inside this entry
                e.querySelectorAll('input, select').forEach(input => {
                    const name = input.getAttribute('name');
                    if(name) {
                        input.setAttribute('name', name.replace(/santri\[\d+\]/, `santri[${i}]`));
                    }
                });
            });
            santriIndex = entries.length > 0 ? entries.length - 1 : -1;
        }
    </script>
</body>
</html>
