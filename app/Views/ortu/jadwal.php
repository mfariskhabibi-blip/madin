...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .page-title { font-size: 1.5rem; color: var(--primary-dark); margin-bottom: 8px; display: flex; align-items: center; gap: 10px; flex-wrap: wrap; font-weight: 700; }
        .page-subtitle { color: var(--gray); font-size: 0.85rem; margin-bottom: 20px; }
        
        /* ANAK LIST */
        .anak-list { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 24px; }
        .anak-tag { background: white; padding: 8px 16px; border-radius: 30px; font-weight: 600; font-size: 0.8rem; color: var(--primary); border: 1px solid var(--primary); box-shadow: var(--shadow); transition: var(--transition); display: inline-flex; align-items: center; gap: 8px; }
        .anak-tag:hover { background: var(--primary); color: white; transform: translateY(-2px); }
        .anak-tag i { font-size: 0.8rem; }
        .anak-tag span { font-weight: normal; opacity: 0.8; }
        
        /* SCHEDULES CONTAINER */
        .schedules-container { display: flex; flex-direction: column; gap: 20px; margin-bottom: 24px; }
        ...
        .day-header { background: var(--light); padding: 12px 20px; border-bottom: 1px solid var(--light-gray); display: flex; align-items: center; gap: 10px; }
        .day-title { font-weight: 800; color: var(--primary); font-size: 0.95rem; text-transform: uppercase; letter-spacing: 1px; }
        .day-header i { color: var(--primary); font-size: 1rem; }
        
        .jadwal-list { display: flex; flex-direction: column; }
        .jadwal-row { display: grid; grid-template-columns: 120px 1fr auto; align-items: center; padding: 14px 20px; border-bottom: 1px solid var(--light-gray); transition: var(--transition); gap: 12px; }
        .jadwal-row:last-child { border-bottom: none; }
        .jadwal-row:hover { background: #fafcff; }
        
        .time-box { background: rgba(26, 95, 180, 0.1); color: var(--primary); padding: 6px 12px; border-radius: 8px; font-weight: 700; font-family: monospace; font-size: 0.85rem; text-align: center; width: fit-content; display: inline-flex; align-items: center; gap: 5px; }
        .time-box i { font-size: 0.75rem; }
        
        .activity-info { padding: 0 12px; flex: 1; }
        .activity-name { font-size: 0.95rem; font-weight: 700; color: var(--dark); margin-bottom: 4px; }
        .activity-details { font-size: 0.75rem; color: var(--gray); display: flex; align-items: center; gap: 16px; flex-wrap: wrap; }
        .activity-details i { color: var(--accent); width: 16px; font-size: 0.7rem; }
        
        .action-btn { color: var(--primary); background: none; border: none; cursor: pointer; padding: 6px 10px; border-radius: 6px; transition: var(--transition); }
        .action-btn:hover { background: rgba(26, 95, 180, 0.1); }
        
        /* EMPTY STATE */
        .empty-state { text-align: center; padding: 60px 20px; background: white; border-radius: var(--radius); box-shadow: var(--shadow); border: 1px solid var(--light-gray); }
        ...
        /* INFO CARD */
        .info-card { margin-top: 24px; background: white; border-radius: var(--radius); padding: 18px 20px; border: 1px solid var(--light-gray); box-shadow: var(--shadow); }
        .info-header { display: flex; align-items: center; gap: 12px; margin-bottom: 12px; }
        .info-icon { width: 32px; height: 32px; border-radius: 8px; background: rgba(14, 165, 233, 0.1); display: flex; align-items: center; justify-content: center; color: var(--info); }
        .info-header h3 { font-size: 0.95rem; font-weight: 700; color: var(--dark); }
        .info-list { margin-left: 44px; font-size: 0.8rem; color: var(--gray); display: flex; flex-direction: column; gap: 6px; }
        .info-list li { display: flex; align-items: center; gap: 8px; }
        .info-list li i { font-size: 0.7rem; color: var(--accent); }
        ...
        @media (max-width: 768px) {
            ...
            .jadwal-row { grid-template-columns: 1fr; gap: 10px; padding: 14px 16px; }
            .time-box { width: fit-content; }
            .activity-info { padding: 0; }
            .activity-details { gap: 10px; flex-direction: column; align-items: flex-start; }
            .anak-tag { padding: 5px 12px; font-size: 0.7rem; }
            .day-header { padding: 10px 16px; }
            .day-title { font-size: 0.85rem; }
            .info-list { margin-left: 0; margin-top: 8px; }
        ...
        @media (max-width: 576px) {
            ...
            .anak-list { gap: 8px; margin-bottom: 16px; }
            .anak-tag { padding: 4px 10px; font-size: 0.65rem; }
            .day-header { padding: 8px 12px; }
            .day-title { font-size: 0.8rem; }
            .jadwal-row { padding: 12px; }
            .time-box { font-size: 0.75rem; padding: 4px 10px; }
            .activity-name { font-size: 0.85rem; }
            .activity-details { font-size: 0.7rem; gap: 8px; }
            .info-card { padding: 14px; }
            .info-list { font-size: 0.7rem; gap: 4px; }
            .info-list li { gap: 6px; }
            .sidebar { width: 260px; }
        }
    ...
    <div class="dashboard-container">
        ...
        <div class="dashboard-content" id="mainContent">
            ...
            <div class="anak-list">
                <?php foreach($anak as $a): ?>
                    <div class="anak-tag">
                        ...
                        <span>(<?= htmlspecialchars($a['nama_kelas'] ?? 'Belum ada kelas') ?>)</span>
                    </div>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
            ...
            <div class="info-card">
                ...
                <ul class="info-list">
                    <li><i class="fas fa-chevron-right"></i> <strong>Jadwal dapat berubah</strong> sewaktu-waktu sesuai dengan kebijakan pondok</li>
                    <li><i class="fas fa-chevron-right"></i> <strong>Waktu kegiatan</strong> mengacu pada jam pesantren (WIB)</li>
                    <li><i class="fas fa-chevron-right"></i> Untuk informasi lebih lanjut, silakan hubungi ustadz pengampu kelas</li>
                </ul>
            </div>
        </div>
    </div>
