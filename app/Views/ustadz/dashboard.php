...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .page-title { font-size: 1.5rem; color: var(--primary-dark); margin-bottom: 20px; display: flex; align-items: center; gap: 12px; flex-wrap: wrap; font-weight: 800; letter-spacing: -0.5px; }
        ...
        .stat-card { background: white; border-radius: var(--radius); box-shadow: var(--shadow); padding: 16px; display: flex; justify-content: space-between; align-items: center; transition: var(--transition); }
        ...
        .stat-label { font-size: 0.7rem; color: var(--gray); font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
        .stat-icon { width: 48px; height: 48px; border-radius: 12px; background: rgba(26, 95, 180, 0.08); color: var(--primary); display: flex; align-items: center; justify-content: center; font-size: 1.4rem; }
        
        /* QUICK ACTIONS */
        .quick-actions { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 24px; }
        .action-card { background: white; border-radius: var(--radius); box-shadow: var(--shadow); padding: 16px; display: flex; align-items: center; gap: 12px; transition: var(--transition); cursor: pointer; }
        .action-card:hover { transform: translateY(-3px); box-shadow: 0 8px 16px rgba(0, 0, 0, 0.1); }
        .action-icon { width: 45px; height: 45px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 1.3rem; color: white; }
        .action-info { flex: 1; }
        .action-title { font-weight: 700; font-size: 0.9rem; color: var(--dark); margin-bottom: 2px; }
        .action-desc { font-size: 0.7rem; color: var(--gray); }
        
        /* WELCOME BANNER */
        .welcome-banner { background: linear-gradient(135deg, var(--primary) 0%, var(--primary-dark) 100%); border-radius: 12px; padding: 24px 28px; margin-bottom: 24px; color: white; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px; box-shadow: 0 10px 20px rgba(26, 95, 180, 0.2); position: relative; overflow: hidden; }
        .welcome-banner::after { content: ''; position: absolute; top: -50%; right: -10%; width: 250px; height: 250px; background: rgba(255, 255, 255, 0.05); border-radius: 50%; }
        .welcome-banner h2 { font-size: 1.4rem; margin-bottom: 6px; font-weight: 700; }
        .welcome-banner p { opacity: 0.9; margin-bottom: 0; font-size: 0.85rem; max-width: 500px; }
        .banner-date { background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); padding: 8px 20px; border-radius: 30px; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 10px; border: 1px solid rgba(255, 255, 255, 0.2); }
        
        /* TABLE STYLES */
        .table-responsive { overflow-x: auto; }
        .table { width: 100%; border-collapse: collapse; }
        .table th { text-align: left; padding: 10px 16px; background: var(--light); font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.5px; color: var(--gray); font-weight: 700; border-bottom: 1px solid var(--light-gray); }
        ...
        .student-name { font-weight: 700; color: var(--primary-dark); font-size: 0.85rem; }
        .student-nis { font-size: 0.65rem; color: var(--gray); font-family: monospace; margin-top: 2px; }
        
        .score-badge { padding: 4px 10px; border-radius: 20px; font-weight: 700; font-size: 0.75rem; background: rgba(229, 165, 10, 0.1); color: var(--accent); display: inline-flex; align-items: center; gap: 4px; border: 1px solid rgba(229, 165, 10, 0.15); }
        
        /* ANNOUNCEMENT STYLES */
        .announcement-item { padding: 14px 18px; border-bottom: 1px solid var(--light-gray); transition: var(--transition); }
        .announcement-item:last-child { border-bottom: none; }
        .announcement-item:hover { background: #f8fafc; }
        .announcement-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; flex-wrap: wrap; gap: 6px; }
        .announcement-title { font-size: 0.9rem; font-weight: 700; color: var(--primary-dark); }
        .announcement-badge { padding: 3px 10px; border-radius: 20px; font-size: 0.65rem; font-weight: 600; background: rgba(229, 165, 10, 0.1); color: var(--accent); display: inline-flex; align-items: center; gap: 4px; }
        .announcement-content { color: var(--dark); font-size: 0.8rem; line-height: 1.5; margin-bottom: 6px; }
        .announcement-date { font-size: 0.65rem; color: var(--gray); display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
        
        /* INFO CARD */
        .info-card { margin-top: 22px; border-top: 4px solid var(--info); }
        .info-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; padding: 20px 24px; }
        .info-item { display: flex; gap: 12px; align-items: flex-start; }
        .info-item i { color: var(--success); margin-top: 3px; font-size: 0.85rem; }
        .info-item span { font-size: 0.8rem; color: var(--gray); }
        .info-item strong { font-weight: 700; color: var(--dark); display: block; margin-bottom: 4px; font-size: 0.85rem; }
        ...
        @media (max-width: 768px) {
            ...
            .welcome-banner { padding: 16px 20px; flex-direction: column; align-items: flex-start; }
            .welcome-banner h2 { font-size: 1.2rem; }
            .welcome-banner p { font-size: 0.8rem; }
            .banner-date { font-size: 0.75rem; padding: 6px 14px; }
            
            /* Convert table to cards on mobile */
            .table thead { display: none; }
            .table, .table tbody, .table tr, .table td { display: block; width: 100%; }
            .table tr { margin-bottom: 12px; padding: 12px; border-radius: 12px; border: 1px solid var(--light-gray); background: #fff; box-shadow: 0 2px 6px rgba(0,0,0,0.04); }
        ...
        @media (max-width: 576px) {
            ...
            .action-icon { width: 40px; height: 40px; font-size: 1.1rem; }
            ...
            <div class="dashboard-grid">
                ...
                <div class="section-card">
                    <div class="card-header">
                        ...
                        <a href="<?= base_url('ustadz/hafalan') ?>" style="color:var(--primary); font-size:0.75rem;">Lihat Semua <i class="fas fa-arrow-right"></i></a>
                    </div>
                                ...
                                <?php if(!empty($recent_hafalan)): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td data-label="Hafalan">
                                            <div style="font-weight: 600; font-size:0.85rem;"><?= htmlspecialchars($h['surah']) ?></div>
                                            <div style="font-size: 0.7rem; color: var(--gray);">Ayat <?= $h['ayat_awal'] ?> - <?= $h['ayat_akhir'] ?></div>
                                        </td>
                                        <td data-label="Nilai"><span class="score-badge"><i class="fas fa-star"></i> <?= number_format($h['nilai'], 1) ?></span></td>
                                        <td data-label="Tanggal" style="color: var(--gray); font-size:0.75rem;"><?= date('d M Y', strtotime($h['tanggal'])) ?></td>
                                    </tr>
                                    <?php endforeach; ?>
                                <?php else: ?>
                ...
                <div class="section-card">
                    ...
                    <?php if(!empty($pengumuman)): ?>
                        ...
                        <div class="announcement-item">
                            ...
                            <div class="announcement-content">
                                <?= nl2br(htmlspecialchars(substr($p['konten'] ?? '', 0, 100))) ?>
                                <?php if(strlen($p['konten'] ?? '') > 100): ?>...<?php endif; ?>
                            </div>
                    ...
                    <?php else: ?>
                        <div style="text-align:center; padding:40px; color:var(--gray);">
                            ...
                            <p style="font-size:0.8rem;">Tidak ada pengumuman terbaru.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="section-card info-card">
                ...
                <div class="info-grid">
                    <div class="info-item">
                        ...
                        <div>
                            <strong>Manajemen Kehadiran</strong>
                            <span>Pastikan absensi diisi setiap hari sebelum kelas dimulai.</span>
                        </div>
                    </div>
                    <div class="info-item">
                        <i class="fas fa-star"></i>
                        <div>
                            <strong>Pencatatan Hafalan</strong>
                            <span>Input nilai kelancaran dan surat yang sedang dihafal.</span>
                        </div>
                    </div>
                </div>
                <div style="padding: 0 24px 20px 24px; border-top: 1px solid var(--light-gray); margin-top: 0; padding-top: 16px;">
                    <p style="font-size: 0.8rem; color: var(--gray);">
                        <i class="fas fa-chalkboard-user"></i> Saat ini Anda mengampu <strong><?= $total_kelas ?? 0 ?> kelas</strong> dengan total <strong><?= $total_santri ?? 0 ?> santri</strong>.
                    </p>
                </div>
            </div>
        </div>
